...
旧版代码里最影响实验解释性的几个问题已经处理掉：

- `main.py` 里 client 划分逻辑写了两遍，前面算一次，后面又覆盖一次
- `dirichlet` 分支写了一半，调用时会直接报错
- `soft_noniid` 分支里 `major_ratio` 被写死成 `0.8`
- 划分函数会偷偷重置全局随机种子，导致“数据划分”影响“模型初始化”，实验结果不稳定且难解释
- 训练入口、实验 sweep、结果汇总都堆在 `main.py` 里，职责混杂

现在这些逻辑都收束到统一的配置和训练/实验模块里了。
